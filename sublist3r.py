...
# Optional third-party modules.  Keep imports lazy-friendly so that
# ``--help`` and module inspection work before dependencies are installed.
try:
...
def hostname_from_link(link):
    """Extract a normalized hostname from a URL or bare hostname."""
    link = link.strip()
    if not link.startswith(("http://", "https://")):
        link = "http://" + link
    return (urlparse(link).hostname or "").rstrip(".").lower()
...
def parse_args():
    ...
    parser.add_argument('-b', '--bruteforce', help='Enable the subbrute brute-force module', action='store_true')
...
class enumratorBaseThreaded(threading.Thread, enumratorBase):
    ...
    def run(self):
        try:
            ...
            self.print_(R + "[!] %s failed: %s" % (self.engine_name, exc) + W)
            return
        ...
        try:
            ...
            for link in links_list:
                ...
                if not link.startswith('http'):
                    link = "http://" + link
                subdomain = hostname_from_link(link)
                if (
                    belongs_to_domain(subdomain, self.domain)
                    and subdomain
        ...
        try:
            ...
            for link in links_list:
                ...
                if not link.startswith('http'):
                    link = "http://" + link
                subdomain = hostname_from_link(link)
                if not belongs_to_domain(subdomain, self.domain):
                    continue
        ...
        try:
            ...
            for link in links:
                ...
                if not link.startswith('http'):
                    link = "http://" + link
                subdomain = hostname_from_link(link)
                if belongs_to_domain(subdomain, self.domain):
...
class NetcraftEnum(enumratorBaseThreaded):
    ...
    def create_cookies(self, cookie):
        ...
        cookies['netcraft_js_verification_response'] = hashlib.sha1(
            unquote(cookies_list[1]).encode('utf-8')
        ).hexdigest()
        return cookies
    ...
    def extract_domains(self, resp):
        ...
        try:
            ...
            for link in links:
                ...
                for subdomain in subdomains:
                    if not belongs_to_domain(subdomain, self.domain) or '*' in subdomain:
                        continue
...
class portscan:
    def __init__(self, subdomains, ports):
        ...
        self.ports = [str(port).strip() for port in ports if str(port).strip()]
        self.lock = None
...
def main(domain, threads, savefile, ports, silent, verbose, enable_bruteforce, engines):
    ...
    enable_bruteforce = enable_bruteforce is True or enable_bruteforce is None
    ...
    if requests is None:
        if not silent:
            print(
                R + "Error: the 'requests' package is required. "
                "Install it with 'python -m pip install requests'." + W
            )
        return []
    ...
    if enable_bruteforce:
        if subbrute is None:
            raise RuntimeError(
                "The 'subbrute' package is required for brute-force mode. "
                "Install it or omit -b."
            )
        if not silent:
        ...
        output = False
        json_output = False
        bruteforce_list = subbrute.print_target(domain, record_type, subs, resolvers, process_count, output, json_output, search_list, verbose)
